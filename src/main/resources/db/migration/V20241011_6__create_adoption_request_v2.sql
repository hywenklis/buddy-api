CREATE TABLE IF NOT EXISTS adoption_request_v2 (
    ...
    shelter UUID NOT NULL,
    creation_date TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_date TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_adoption_request_v2_pet FOREIGN KEY (pet_id) REFERENCES pet(pet_id),
    CONSTRAINT fk_adoption_request_v2_profile FOREIGN KEY (profile_id) REFERENCES profile(profile_id),
    CONSTRAINT fk_adoption_request_v2_shelter FOREIGN KEY (shelter) REFERENCES shelter_v2(shelter_id)
);
...
COMMENT ON COLUMN adoption_request_v2.shelter IS 'Foreign key linking to the shelter_v2 table';
