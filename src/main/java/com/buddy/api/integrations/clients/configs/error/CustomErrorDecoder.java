...
public class CustomErrorDecoder implements ErrorDecoder {
    ...
    public Exception decode(final String methodKey, final Response response) {
        ...
        return switch (response.status()) {
            case 400 -> new ClientBadRequestException(errorMessage, responseBody, response.status());
            case 401, 403 -> new ClientUnauthorizedException(errorMessage, responseBody, response.status());
    ...
    private String readErrorBody(final Response response) {
        try {
            if (response.body() != null) {
                return IOUtils.toString(response.body().asInputStream(), StandardCharsets.UTF_8);
            }
        } catch (IOException e) {
            log.error("Falha ao ler o corpo da resposta de erro do Feign.", e);
        }
        return "Falha ao ler o corpo da resposta.";
    }
}
